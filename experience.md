...
## 2026-04-06 评分语义不清和 raw JSON 展示会让 submission 页面误导用户

问题：
submission 详情页同时展示 `Public Primary`、`Hidden Score` 和原始 evaluation JSON，但没有解释三者关系；对 `grid-routing` 这类题，用户很容易把 `shown_results` 里的单 case 分数当成总体分，从而误解页面上的总分和排行榜。

如何解决：
把 submission 页改成结构化评测面板，显式展示 `Public Score`、`Shown Average`、hidden/official dataset summary 和 shown case breakdown，并补一句说明当前 public score 对现有题包通常取 hidden dataset aggregate，不再直接暴露 raw JSON。

如何避免：
后续只要页面同时承载“诊断明细”和“用于排序或主展示的总分”，就必须把字段语义写清楚，并优先用结构化 UI 呈现；不要把内部 JSON 直接扔给用户，让用户自己猜哪一个才是最终分数。

## 2026-04-06 集成测试不要把 seed 数据数量写死

问题：
`apps/api/test/agent-submission.integration.test.ts` 仍假设公开题目列表只有一个 `sample-sum`，当仓库新增 `grid-routing` 后，测试即使在业务无回归的情况下也会失败。

如何解决：
把断言改为检查返回集合里包含当前关键题目，而不是写死列表长度；这样仍能验证授权与题目返回逻辑，同时兼容 seed 数据扩展。
